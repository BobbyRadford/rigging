...
- `global/secrets.toml`: names (not values) of secrets every machine carries, e.g. `FILE_HOST_TOKEN`. Values live in `~/.rig/secrets.json` on each machine via `bin/rig secret set NAME` (value on stdin); apply injects them into `env` in Claude's settings.json and `[shell_environment_policy.set]` in Codex's config.toml so agent shells see them, and substitutes `${NAME}` wherever a harness config file references one (this is how the CLIProxyAPI token reaches both harnesses). `bin/rig status` reports missing ones.
