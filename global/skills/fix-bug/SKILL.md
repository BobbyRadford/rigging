...
| `production` | `balanced` + after Step 7 (propose fix), write a regression test that fails on the pre-fix code and passes on the post-fix code. |
...
After the fix is implemented and verified, run only the checks whose gates match the patch. Honor `skip=` from the caller. Do each as a focused review pass over the diff:

- **Contracts** — when the patch changes a client/server, route/schema, IPC, DTO, generated-client, server-function, OpenAPI, tRPC, or API boundary: confirm both sides of the boundary agree.
- **Concurrency** — when the patch touches mutations, jobs, webhooks, retries, idempotency, transactions, async UI writes, polling, or stale async responses: look for races, double-execution, and lost updates.
- **Observability** — after critical-path async/error/integration fixes: confirm the failure path now logs or reports enough evidence to diagnose a recurrence.
- **Security regression** — when the patch touches auth, payments, secrets/env, file upload, webhook signing, external APIs, unsafe redirects, or user-rendered HTML.
- **Error boundaries** — when the bug or fix affects a user-facing failure path, route loader, form submit, background failure surfaced in UI, or blank-screen risk.
- **Migrations** — when the fix requires a corrective migration (missing column, NOT NULL without backfill, missing index, enum value never written, broken default, orphaned rows): plan the multi-phase deploy and audit data integrity instead of hand-waving it.
- **Domain-model mismatch** — when the root cause is not a local bug but enum/state/vocabulary drift or a state machine whose code and business meaning diverged: fix the model, not the symptom.

In `production`, also write the regression test unless explicitly skipped.
...
Once the bug is fixed and verified, run the `simplify` skill on the diff to clean up any duplication, magic numbers, or quick-fix shortcuts the patch introduced — without changing behavior.
